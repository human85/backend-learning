...
## 2026-09-09｜Smoke test 清理证据边界复测

- 学习者独立判断：不能仅凭接口状态码标记清理成功；接口响应不能完全代表数据库真实状态，仍需要查询验证。该回答准确覆盖 HTTP 证据与数据库证据的边界。
- Agent 补充：数据库查询暂时不可用时，结果应标记为“未验证”，保留 projectId、userId、Session 标识和请求 Request ID，待数据库恢复后定向核对；不能把未知写成成功。
- 本次为问答复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜线上版本证据边界复测

- 场景是部署平台显示成功，线上 `/health` 与 `/ready` 均正常，但无法确认实例对应哪个 commit SHA。学习者判断不能直接执行 smoke test，因为版本不匹配时检测没有意义；该回答正确覆盖线上行为与本次改动之间的归因边界。
- Agent 补充：线上验收开始前至少要保留部署版本/commit、目标环境和配置来源等证据，再用 `/health`、`/ready`、可清理业务请求和 Request ID 日志形成完整链路。平台“部署成功”只说明某次构建完成，不自动证明当前请求来自目标版本。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜Smoke 清理失败处置边界复测

- 场景是版本已确认，`/health`、`/ready` 和注册、登录、创建、删除、注销接口均符合预期，但数据库核对发现临时 Session 仍存在。学习者判断不能标记为通过，应查询数据库状态并清理脏数据；该回答正确覆盖业务链路证据与清理验收证据的区别。
- Agent 补充：处理时保留临时对象 ID、唯一邮箱和各请求 Request ID，先确认残留 Session 是否属于本次 smoke，再执行定向注销/销毁或受控管理清理，最后重新查询项目、用户和 Session。清理前后的证据应分别记录，不能用“接口都成功”覆盖残留数据。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜Smoke 请求失败与最终状态复测

- 场景是第一次 `DELETE` 返回 `500`，重试返回 `204`，最后项目、用户和 Session 都已不存在。学习者回答可以通过，因为数据确实已经清理；这只证明最终资源状态，未覆盖第一次请求失败及其根因。
- Agent 补充：应分别记录“最终状态已恢复”和“smoke 请求合同未全部通过”。先用第一次响应的 Request ID 核对 DELETE 的 route、statusCode、durationMs、认证用户和数据库状态，定位 500 的业务或依赖原因；修复或解释后重新执行完整 smoke。幂等性可以成立，但不能替代本次验收的失败证据。
- 本次为预测复测，发现该边界仍需间隔复查；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-09｜Smoke 失败记录保留复测

- 场景是第一次 `500` 查明为短暂数据库故障，修复后完整 smoke 全部通过。学习者判断应保留第一次失败记录，说明后续出现问题时可据此排查；该回答正确覆盖故障时间线和验收结果不可互相覆盖的边界。
- Agent 补充：最终报告应同时保留失败运行的时间、版本、Request ID、错误分类和修复动作，以及重新运行的版本和通过证据。历史失败不能删除或改写成成功；当前状态与过程证据要分开表达。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜DELETE 404 排查链路复测

- 场景是删除返回 `404`，但数据库中项目仍存在且归属当前用户。学习者回答应通过 Request ID 找到对应日志，核对 route、statusCode、method 等，再进行问题定位和代码检查；核心排查入口正确。
- Agent 补充：还要把日志中的请求路径/参数映射到保存的 `projectId`，核对认证 Session 的 userId、数据库项目 owner 和实际路由参数转换结果，才能区分传错 ID、路由选择错误、归属条件 bug 或 Service 查询 bug。项目仍存在时不能把这个 `404` 解释为删除成功。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜修复后完整回归边界复测

- 场景是已定位并修复 DELETE 路由拼接错误。学习者判断测试和 smoke 都要完整走一遍，因为修改可能带来其他副作用；该回答正确覆盖局部修复验证与端到端流程验收的分层关系。
- Agent 补充：先跑受影响的单元/e2e 缩短反馈，再按完整顺序执行生产 smoke，重新验证认证、Session 恢复、项目归属、删除和数据库清理。只重复失败的 DELETE 只能证明一个请求暂时恢复，不能证明整条链路没有回归。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜R2 退出条件边界复测

- 场景是本地测试和 CI 均通过，部署版本已确认，但生产 smoke 尚未执行。学习者判断不能标记 R2 完成，因为缺乏真实环境 test；该回答正确区分隔离验证与真实部署验收。
- Agent 补充：退出证据还需包含线上目标版本、`/health`、`/ready`、可清理业务链路、Request ID 到日志的关联，以及项目、用户和 Session 的数据库状态核对。历史线上验收不能替代本轮改动的生产证据。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜线上配置证据边界复测

- 场景是执行线上 smoke 前需要确认 `DATABASE_URL`、`SESSION_SECRET` 等配置。学习者判断不能打印配置原文以免泄露，并认为连接成功可代表配置正确；前半正确，后半只适用于数据库连接配置。
- Agent 补充：应只记录配置名、环境和脱敏摘要。`DATABASE_URL` 可由应用建连或 `/ready` 的 `SELECT 1` 验证；`SESSION_SECRET` 要通过登录、Cookie 签名和刷新恢复验证；`FRONTEND_ORIGIN` 要通过 CORS 预检和凭证响应头验证。单一数据库连接不能代表全部配置正确。
- 本次为预测复测，配置证据边界仍需间隔复查；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-09｜配置 fail-fast 边界复测

- 场景是 `SESSION_SECRET` 缺失或长度不合规。学习者判断应用应在启动阶段直接失败，因为继续启动会让请求必定失败并浪费资源；该回答正确覆盖必需配置的 fail-fast 原则。
- Agent 补充：配置校验应发生在应用监听端口前，进程只输出安全的配置项名称和校验错误，不输出密钥原文。启动失败与运行期业务 500 是两种不同的故障边界。
- 本次为预测复测，没有部署、线上请求或代码修改；R2 工程退出仍待生产 smoke test。

## 2026-09-09｜浏览器 CORS 验收边界复测
...
## 2026-09-09｜CORS 响应头可见性复测

- 场景是允许来源和 Cookie 凭证配置正确，但没有暴露 `X-Request-ID`。学习者判断请求会成功，只是前端拿不到 ID；该回答正确区分请求处理结果与浏览器脚本可见性。
- Agent 补充：服务端仍会写入带 Request ID 的日志，浏览器开发者工具可能显示响应头，但 `fetch`/前端代码读取会受到 CORS 暴露规则限制。缺少暴露配置会削弱客户端向服务端反馈问题时的关联能力，不等于 API 请求本身失败。
- 本次为独立预测复测，响应头可见性边界通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-09｜CORS 响应头配置复测
...
## 2026-09-10｜反向代理协议识别边界复测

- 场景是 HTTPS 在反向代理处终止，应用内部连接为 HTTP，需要解释 `X-Forwarded-Proto`/`trust proxy` 的作用及盲目信任风险。学习者回答其目的是让 HTTP 环境也能携带 Cookie，并指出用户 Cookie 泄露风险；风险方向正确，但前半混淆了代理内部链路与浏览器传输规则。
- Agent 补充：正确配置让应用识别外部 HTTPS，从而设置 `Secure` Cookie 和执行安全策略；浏览器仍只在 HTTPS 请求中发送该 Cookie。应用只应信任预期数量或地址范围内的代理，不能无条件接受客户端伪造的转发头。
- 本次为提示式复测，代理协议识别与信任边界需后续间隔复查；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜反向代理基础补课

- 学习者指出此前没有学习反向代理、公网、转发头和 `trust proxy`，不理解相关题目。此前题目不计入能力测评，先补充基础概念。
- 基本链路是浏览器通过公网 HTTPS 访问反向代理，代理可能终止 TLS，再通过内部 HTTP 转发给 Nest；`X-Forwarded-Proto` 用于传递外部协议，`trust proxy` 控制应用是否相信受信任代理提供的元数据。它不改变浏览器发送 `Secure` Cookie 的 HTTPS 规则。
- 本次为概念补课，没有独立回答或部署、线上请求；后续先复述链路，再复测代理信任边界，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜反向代理基础链路复述

- 学习者正确复述：反向代理传给 Nest，告诉 Nest 外部请求通过 HTTPS 到达；并能对应到代理到 Nest 的内部 HTTP 链路。
- 该复述通过基础概念检查，尚未验证 `trust proxy` 的信任范围和伪造转发头风险；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜`trust proxy` 配置补课
...
## 2026-09-10｜反向代理拓扑验证复测

- 学习者判断：若平台明确保证应用前有一层受信任代理，`trust proxy = 1` 与拓扑匹配；还要验证该反向代理真实存在。该回答正确抓住配置必须有运行拓扑依据。
- Agent 补充：验证应从公网请求观察 HTTPS 入口、代理转发的 `X-Forwarded-Proto: https` 和应用发出的 `Secure` Cookie，并结合平台部署配置确认只有预期的一层代理；不能只看代码中的数字或平台名称。
- 本次为独立基础复测，拓扑验证边界通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜受保护业务链路验收边界复测

- 学习者补充：确认公网登录后，还要调用其他需要登录凭证的接口，检查 Cookie 是否能正确送达后端服务。该回答正确把 Session 验证从单一登录/刷新扩展到真实受保护业务链路。
- Agent 补充：至少应覆盖一个项目列表或创建接口，并核对响应中的用户归属、Request ID 日志和数据库状态；这样可以验证 Cookie 发送、Session 解析、Guard、归属过滤和业务查询共同工作。
- 本次为独立预测复测，受保护业务链路验收边界通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜R2 进度核对与下一步

- 学习者正确指出真实环境测试仍是 R2 缺口；本次又能独立说明登录后要调用受保护业务接口验证 Cookie 到达后端，受保护链路验收边界通过。
- 工程进度口径：Request ID/结构化日志、数据库 Readiness、基础 CI 三个主目标已有实现和验证，生产 smoke 尚未执行，按四个主目标约为 75%；路线图的三个工程退出条件中前两项已有证据，按门槛为 2/3（约 67%）。最新 [GitHub Actions run 34446818133](https://github.com/human85/backend-learning/actions/runs/34446818133) 全部通过。
- 学习进度不直接换算成工程百分比：Request ID、Readiness、CI 和清理证据边界已有多次独立判断；反向代理刚完成基础补课，`trust proxy` 信任范围仍需间隔复测；线上真实 smoke 尚未提供学习和工程证据。
- 下一步计划：先确认公开 HTTPS API 地址、部署 commit 和一层代理拓扑；再执行 `/health`、`/ready`、注册/登录/刷新 `/auth/me`、受保护项目接口、删除/注销和数据库清理核对。失败时保留 Request ID 与时间线，定位后跑受影响测试、完整测试和完整 smoke；全部通过后更新 R2 状态，再开始 R3 陌生需求拆解。

## 2026-09-10｜开发/生产代理差异复测

- 场景是解释为什么当前项目只在生产设置 `trust proxy = 1`。学习者判断开发环境没有反向代理；该回答正确覆盖本机直连与生产代理转发的差异。
- Agent 补充：开发环境直接访问本机应用，不应无依据地信任客户端转发头；生产配置依赖平台确实提供一层受信任入口，不能把生产数字复制到所有环境。
- 本次为独立基础复测，环境差异边界通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜转发头信任边界复测

- 场景是开发环境直接访问 Nest，但请求手动带 `X-Forwarded-Proto: https`。学习者判断应忽略该头，并认为当前设置没有意义；忽略结论正确，需补充它只是在无受信任代理的开发拓扑中不应被采用。
- Agent 补充：没有 `trust proxy` 时，应用不应把客户端自带的转发头当作代理证明；生产若有真实代理，才按层数/地址范围选择信任策略。转发头本身不是安全凭证。
- 本次为独立基础复测，开发环境转发头边界通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜多层代理拓扑判断复测

- 场景是生产链路增加 CDN，形成“浏览器 → CDN → 反向代理 → Nest”。学习者判断 `trust proxy = 1` 不一定正确，因为中间多了一层代理，并提出先确认前一个代理是否安全；核心判断正确。
- Agent 补充：还要确认应用到各代理的实际 hop 顺序、每层是否覆盖或追加 `X-Forwarded-*`、代理路径是否固定以及可信 IP/网络范围。数字信任值不能脱离拓扑单独决定，不能只把 `1` 机械改成 `2`。
- 本次为独立基础复测，多层代理判断通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜可变代理路径风险复测

- 场景是一部分请求经过一层代理，另一部分经过两层代理，却固定设置 `trust proxy = 2`。学习者判断不安全，可能出现伪造额外一层代理作恶的风险；该回答正确抓住短路径下的信任边界问题。
- Agent 补充：应固定公网入口和代理链，或只信任已知代理地址/网络范围，并确认代理会覆盖客户端提供的转发头。不要把“最大可能层数”当作充分的安全配置。
- 本次为独立基础复测，可变路径风险判断通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜部署证据前置条件复测

- 场景是准备执行本轮改动的生产 smoke，但缺少当前公网地址和部署版本证据。学习者判断不能直接执行，因为可能命中错误部署地址或旧版本，测试没有意义；该回答正确覆盖验收归因前提。
- Agent 补充：开始请求前应保存目标 HTTPS API 地址、部署 commit/版本、目标环境和代理拓扑；再确认 `/health`、`/ready` 与受保护业务链路。没有这些证据时只能把 smoke 标为未执行，不能借用历史线上结果。
- 本次为独立预测复测，部署前置条件通过；没有部署、线上请求或代码修改，R2 工程退出仍待生产 smoke test。

## 2026-09-10｜前端 API 接入与线上地址澄清
...
- 用户提供并确认公网 API 地址为 `https://backend-learning-mini-sass-api.onrender.com`。本次业务 smoke 针对 Render API 服务和静态站点的 `19da815`；注意服务名和前端服务中的 `saas`/`sass` 拼写必须逐字符核对，静态站点 Rewrite 的目标是 `backend-learning-mini-sass-api`。随后仅变更文档的 `611484b` 已在两项服务成功自动部署。
...
## 2026-09-10｜部署平台与地址记录

- 学习者指出部署平台、环境和地址需要做好记录。新增 [Mini SaaS 部署清单](notes/deployment-inventory.md)，集中记录 Render Production、GitHub 仓库与 `main` 分支、API/前端 Service ID、两个公网地址、静态站点 Rewrite、配置键名、数据库服务和验证边界。
- 清单只保留可公开的定位信息，不记录 `DATABASE_URL`、`SESSION_SECRET`、数据库密码或其他密钥；“核对时线上 commit”和“本轮业务 smoke 目标 commit”分开记录，避免把文档部署版本当成业务验收版本。
- 本次只维护部署档案和项目索引，没有修改业务代码，也没有新增独立能力测评；Prettier、`git diff --check`、提交和推送均已完成。下一课按 R3 先拆解陌生需求的数据关系、约束、权限、API 行为和失败场景。
