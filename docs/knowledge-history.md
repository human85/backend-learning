...
本表从 2026-09-02 的学习进度快照迁出，保留当时的程度与证据，供复习和查找旧课程使用。它不是当前能力评级，也不代表本次重新执行过测试或验收。

- “接触过 / 理解中”沿用原记录，不自动换算成新的独立判断等级。
- 部分证据混合了实现结果与学习者回答，不能仅凭测试通过认定学习者掌握。
- 表内“尚未实现”等表述以当时为准；当前焦点和后续变化以 [学习进度](learning-progress.md)、[项目档案](projects/) 和 [学习日志](learning-log.md) 为准。
- 新课程与复测结果写入当前进度和日志，不继续扩充这张历史表。完整源码和笔记可按知识点检索。
