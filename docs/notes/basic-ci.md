...
该数值不是通用安全默认值，必须与实际网络拓扑一致。若应用直连公网而没有受信任代理，应关闭或收紧代理信任，避免接受客户端伪造的 `X-Forwarded-*` 信息；只有明确存在一层平台代理时才使用 `1`。

验收时要从公网运行证据确认这层代理确实存在：HTTPS 入口经过代理，代理转发外部协议，应用据此发出 `Secure` Cookie。代码中的 `trust proxy = 1` 单独不能证明部署拓扑正确。

增加 CDN 或其他入口后，不能只按代理数量修改数字。要确认应用实际看到的 hop 顺序、每层对 `X-Forwarded-*` 的追加/覆盖、路径是否固定和可信网络范围，再选择 hop 数或地址函数。

若代理路径有时一层、有时两层，固定信任最大层数会留下短路径伪造空间。应固定入口，或按受信任代理的地址/网络范围配置，并让代理覆盖客户端提交的转发头。

生产 smoke 开始前必须保存目标公网 HTTPS 地址、当前部署 commit/版本、目标环境和代理拓扑；缺少任一关键归因证据时，不应把请求结果写成当前改动的验收结果。

确认流程可分为四层：Render 部署记录核对 URL 与 commit；浏览器 Network 核对前端 `/api` Rewrite、Set-Cookie 和受保护请求；公网 HTTPS 检查 `/health`、`/ready` 与业务链路；Neon 按本次对象 ID 查询清理。内部 `X-Forwarded-Proto` 通常通过拓扑配置和 Secure Cookie 行为间接验证，不应为观察它而在生产日志中打印敏感头。

开发环境通常由浏览器直接访问本机 Nest，没有前置代理，因此不设置 `trust proxy`；生产环境若由平台代理转发，才按实际代理层数配置。不要把生产代理信任设置无条件复制到本机或直连公网环境。

开发环境请求中即使出现 `X-Forwarded-Proto`，也不能把它当作 HTTPS 证明；没有受信任代理时应忽略客户端自带的转发头。生产才根据真实代理链配置信任范围。

登录后不能只验证 `/auth/me`；还应调用至少一个受保护业务接口，检查 Cookie 到达后端、Session Guard 通过、资源归属条件生效，并用 Request ID 和数据库状态补齐证据。
