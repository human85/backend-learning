...
| 幂等请求与联合唯一约束               | 理解中 | 已理解客户端复用幂等键应对超时重试，按 `(userId, operation, key)` 隔离，并用请求哈希识别错误复用；幂等记录需要持久化和保留窗口       |
...
完成第二轮可靠性小节的概念训练：限流与重试、幂等键、事务性 Outbox、消息确认、死信队列和异步任务状态；尚未把这些概念实现为项目代码。
...
目前没有已记录的阻塞。Mini SaaS 当前不是完整生产就绪系统：登录限流、Request ID、结构化日志、指标告警、数据库 Readiness、备份恢复演练、自动化生产 migration、性能容量和正式回滚演练，以及幂等/Outbox/队列代码尚未实现，将在第二轮按优先级补充。
