...
- 实践进度：Projects CRUD 已持久化；User Entity 与第二份 migration 已建立，开发库和测试库都已创建 `users` 表并验证邮箱唯一约束
...
实现用户注册：定义输入与公开响应，规范化邮箱，选择密码哈希算法，并将数据库唯一冲突映射为清晰的 `409 Conflict`。

## 下一步完成标准

- 注册只接受合法邮箱和满足最低要求的密码，不接收 `passwordHash`。
- 数据库只保存规范化邮箱和密码哈希，注册响应只返回公开字段。
- 普通重复注册与并发唯一冲突都返回 `409`，而不是泄漏数据库异常形成 `500`。
- 单元测试验证业务规则，e2e 验证真实哈希、唯一约束和响应中不含敏感字段。
