...
进入用户与项目归属：设计 User 表，理解一对多关系、外键和唯一约束，为注册、登录和“用户只能访问自己的项目”建立数据基础。

## 下一步完成标准

- 能说明 User 与 Project 的一对多关系，以及外键解决的数据一致性问题。
- 能解释为什么邮箱既需要 API 格式校验，也需要数据库唯一约束。
- 创建 User Entity 和迁移，并让 Project 通过 `ownerId` 归属于用户。
