...
| 幂等请求与联合唯一约束               | 理解中 | 已完成 Hono + Drizzle 实验：联合唯一索引仲裁并发，请求哈希区分重放与错误复用；HTTP 编排和过期回收未接入                              |
...
完成幂等记录与联合唯一约束的小实验，继续学习第二轮可靠性小节：Transactional Outbox、消息确认、死信队列和异步任务状态；HTTP 层幂等编排、Outbox、队列和任务代码仍未实现。
...
目前没有已记录的阻塞。Mini SaaS 当前不是完整生产就绪系统：登录限流、Request ID、结构化日志、指标告警、数据库 Readiness、备份恢复演练、自动化生产 migration、性能容量和正式回滚演练，以及 HTTP 层幂等、Outbox/队列代码尚未实现，将在第二轮按优先级补充。
