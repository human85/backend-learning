...
- 阶段：最小 Drizzle + PostgreSQL 纵向切片已完成；幂等、Outbox 失败回滚及单 Worker 基础处理已验证，R1 重复投递实验待完成
...
- 现有集成用例包含成功落库、owner 过滤、重放、内容冲突、并发、Outbox 写入失败后的整体回滚和恢复重试，以及 Worker 成功、重试、失败上限和重启继续处理；当前共 14 项数据库集成测试通过。当前 Worker 没有领取锁，只适用于单 Worker 教学实验，不代表多实例安全。
...
下一课验证重复投递并用稳定事件 ID 去重，范围、退出条件及暂缓内容见 [学习进度](../learning-progress.md) 和 [路线图](../roadmap.md)。当前固定教学身份和单库实验不构成生产认证或生产可靠性承诺。
