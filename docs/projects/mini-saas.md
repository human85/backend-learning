...
Mini SaaS 已满足第一轮“能够借助 AI 从需求走到线上，并能审查常见后端边界”的目标，但不宣称完整生产就绪。R2 已补充 Request ID、请求完成 JSON 日志、数据库 Readiness、基础 CI，并在 Render `19da815` 部署版本上完成公网 smoke；尚未实现登录限流、指标告警、备份恢复演练、自动化生产 migration、性能容量和正式回滚演练。

## 下一项应用课程

Hono 最小对照切片和 R1 可靠性实验已经完成。Mini SaaS 的 R2 排错与基础交付课程已完成，下一课进入 R3，通过一个陌生业务需求验证独立拆解能力。具体范围及退出条件见 [路线图](../roadmap.md)。
...
- 使用路由模板代替原始 URL，不记录请求体、认证凭证和原始异常；安全异常过滤器保持已知 4xx 响应，未知错误返回通用 500。
- 无数据库 HTTP 测试覆盖并发编号隔离、CORS 响应头、401/404/500、204 与非法 JSON；完整验证结果见学习日志。
- 当前仅记录请求完成；没有 Service 内部日志上下文、错误原因分类和客户端断连日志；生产 smoke 已验证请求头和 Render 日志关联，但仍不宣称已具备依赖故障根因定位。

## 2026-09-08｜数据库 Readiness

- 新增 GET /ready：独立 pg 探测池执行 SELECT 1，返回 200 或 503，禁止缓存；保留静态 /health。
- 专用池最多一条连接，并发请求共享在途探测；连接超时 1000ms、语句超时 500ms、客户端查询超时 750ms，失败连接销毁；启用应用关闭钩子清理探测池。
- 请求完成日志新增白名单分类 database_probe_failed，与响应 Request ID 关联；仅识别依赖探测失败，不声称已区分网络、认证和 SQL 根因。
- 本机真实 PostgreSQL 和测试专属 TCP 代理验证连接无响应、查询响应中断、恢复及慢 SQL 取消；46 个单元测试、43 个 HTTP e2e、build 与变更文件检查通过。
- 不探测业务连接池饱和、schema、写权限；本课未修改平台健康探测配置，生产 smoke 已验证部署后的 `/ready`，这些边界仍未覆盖。
...
- 本次 smoke 覆盖真实部署和数据清理，但不代表完成登录限流、监控告警、备份恢复、自动生产 migration 或容量评估。
