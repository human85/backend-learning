...
## D-020｜生产 schema 变更默认保持向后兼容

- 日期：2026-08-12
- 状态：有效
- 决策：可能与在线旧版本并存的数据库变更采用扩展—迁移—收缩流程；新增字段、数据回填、代码切换和删除旧字段应拆分，禁止把破坏性字段改名或删除与依赖它的新代码视为一次原子部署。
- 原因：应用部署和数据库 migration 不是同一个事务，旧请求、滚动部署和代码回滚都可能继续运行旧 SQL；备份恢复成本高且可能丢失恢复点后的数据。
- 影响：新增必填列时先允许 `NULL` 或提供安全回填方案；删除旧字段前确认所有运行版本均已停止使用；备份必须存在并可验证，但不代替兼容 migration 设计。

## D-021｜Hono 对照项目使用稳定 Drizzle 和显式 migration

- 日期：2026-08-13
- 状态：有效
- 决策：Hono 对照项目使用当前稳定版 Drizzle ORM，不追随 1.0 预发布版本；数据库结构采用 code-first schema → `generate` → 审查 SQL → `migrate`，不以 `push` 作为正式变更流程。
- 原因：对照项目需要稳定、可重复的工具链，同时学习者要建立“代码描述不等于数据库已变化”的迁移意识，并能直接阅读实际 DDL。
- 影响：生成的 SQL、snapshot 和 journal 应提交；数据库地址只放在未提交的环境配置中；变更 schema 后必须先审查新 migration，再应用到目标数据库。Hono workspace 固定兼容的 TypeScript 版本，第三方声明问题只允许通过局部、可解释的编译配置处理。

## D-022｜第二轮按有退出条件的阶段推进

- 日期：2026-09-07
- 状态：有效，细化 D-005 与 D-007
- 决策：Mini SaaS 承担真实业务、认证、浏览器与部署；Hono + Drizzle 承担框架对照和有界的数据库可靠性实验。第二轮先收尾 R1，再进入排错/基础 CI 与陌生需求交付，其他主题按具体问题选入。
- 原因：第一轮已完成，继续沿技术清单扩展容易失去终点并重复维护两套应用；阶段应同时证明工程结果和可迁移的判断力。
- 影响：范围、预计课程数和退出条件统一放在 `roadmap.md`；当前课只在 `learning-progress.md` 维护。不因学习 Worker 就自动引入 Redis、多消费者或完整队列平台；架构重构要服务当前规则或已出现的误用风险。

## D-023｜先判断后反馈，分开保存工程与学习证据

- 日期：2026-09-07
- 状态：有效，细化 D-002 与 D-006
- 决策：沿用 AI 主导实现，按 `learning-method.md` 加入回忆、预测、故障验证、迁移和短复测；工程验证与学习者回答分别记录。
- 原因：运行结果不能证明独立能力，长知识表也不能替代实际测评；需要用新条件下的判断检验理解。
- 影响：不给测评题提前泄露答案；用户要求直接演示时继续工作并记为未测评。历史知识表迁出当前快照，保留原始证据，不自动换算掌握等级；纯文档维护不提升能力状态。
