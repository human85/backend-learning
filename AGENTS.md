...
- 当前第一优先级是在 30 天内跑通需求、前端、API、认证授权、数据库、测试、Docker、部署和日志排错的最小全栈闭环。第一轮对非关键细节采用时间盒，达到“能解释、能审查、能在当前项目使用”后继续推进；不得为了速度跳过输入校验、数据正确性、权限边界、密钥安全和部署验证。
- 第一轮闭环完成后再进行第二轮深化，系统补充事务与并发、索引与性能、Redis、队列、复杂测试和生产可靠性，不在第一轮提前展开这些主题。
